...
namespace DrkCraft
{
    ...
    void SettingsMenu::on_render(void)
    {
        ...
        if (ImGui::TreeNode("Video"))
        {
            ...
            const auto& monitors = Application::get_monitor_manager().monitors();
            ...
            if (ImGui::DragInt("Render Distance", &m_settings.video.render_distance, 1.0f, 4, 64));
                make_dirty(Setting::RenderDistance);
    ...
    void SettingsMenu::apply(void)
    {
        ...
        if (dirty(Setting::RenderDistance))
        {
