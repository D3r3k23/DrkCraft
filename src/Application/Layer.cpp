...
namespace DrkCraft
{
    ...
    {
        DRK_LOG_CORE_INFO("Creating Layer: {}", get_layer_name());
        if (activate)
            activate_layer();
    }
    ...
    Layer::~Layer(void)
    {
        DRK_LOG_CORE_INFO("Deleted Layer: {}", get_layer_name());
    }

    void Layer::attach_layer(void)
    {
        DRK_LOG_CORE_TRACE("Attaching Layer: {}", get_layer_name());
    ...
    {
        DRK_LOG_CORE_TRACE("Detaching Layer: {}", get_layer_name());
        m_layerAttached = false;
        m_layerActive = false;
        on_detach();
    }
    ...
    {
        DRK_LOG_CORE_TRACE("Activating Layer: {}", get_layer_name());
        m_layerActive = true;
    }

    void Layer::deactivate_layer(void)
    {
        DRK_LOG_CORE_TRACE("Deactivating Layer: {}", get_layer_name());
        m_layerActive = false;
    }
